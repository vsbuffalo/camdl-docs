"""Rising-edge holdout: fit on days 0-3, score on days 4-13."""
import sys
...
# From camdl pfilter (5000 particles, seed 1)
results = [
    # (label, k, train_ll, holdout_ll)
    ("Poisson\nfixed IC",                 2, -11.98, -108.89),
    ("NegBin\nprofile-k MLE",             3, -11.94,  -54.66),
    ("Poisson + β proc-noise\nσ_se profile MLE", 3, -12.14,  -81.65),
]
...
for ax, vals, ttl in [
    ...
    (axes[1], hold,  "Holdout log-lik\n(days 4–13, 10 obs) — out-of-sample"),
    (axes[2], total, "Total log-lik (train + holdout)"),
]:
    ...
    ax.set_ylim(lo - pad*1.3, hi + pad*1.5)
    for i, v in enumerate(vals):
        ax.text(i, v + pad*0.2, f"{v:.2f}", ha="center", fontsize=9)
    best = int(np.argmax(vals))
    ax.scatter([best], [vals[best] + pad*0.8], marker="*", s=200,
               color="red", edgecolor="white", linewidth=0.8, zorder=5)
    ax.spines[["top","right"]].set_visible(False)

fig.suptitle("Rising-edge holdout — fit on days 0–3 (pre-acceleration), score on days 4–13",
             fontsize=11, y=1.02)
fig.tight_layout()
fig.savefig(HERE.parent/"figures/holdout_rising_comparison.png",
            bbox_inches="tight")
plt.close(fig)
print("wrote figures/holdout_rising_comparison.png")
